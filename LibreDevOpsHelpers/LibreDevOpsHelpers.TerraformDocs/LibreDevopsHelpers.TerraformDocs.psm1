...
function Format-TerraformCode {
    ...
    Format-Terraform -CodePath $CodePath      # existing helper

    # ── step 2: sort variables.tf ───────────────────────────────────────
    $varPath = Join-Path $CodePath $VariablesFile
    if (Test-Path $varPath) {
        $sorted = Format-TerraformVariables `
                    -VariablesContent (Read-TerraformFile $varPath)
        Write-TerraformFile -Filename $varPath -Content $sorted
    }

    # ── step 3: sort outputs.tf ─────────────────────────────────────────
    $outPath = Join-Path $CodePath $OutputsFile
    if (Test-Path $outPath) {
        $sorted = Format-TerraformOutputs `
                    -OutputsContent (Read-TerraformFile $outPath)
        Write-TerraformFile -Filename $outPath -Content $sorted
    }
}
...
function Read-TerraformFile {
    ...
    try   { return Get-Content -Raw -LiteralPath $Filename }
    catch { _LogMessage -Level ERROR -Message $_.Exception.Message -InvocationName $MyInvocation.MyCommand.Name; throw }
}
...
function Write-TerraformFile {
    ...
    try   { $Content | Set-Content -LiteralPath $Filename }
    catch { _LogMessage -Level ERROR -Message $_.Exception.Message -InvocationName $MyInvocation.MyCommand.Name; throw }
}
